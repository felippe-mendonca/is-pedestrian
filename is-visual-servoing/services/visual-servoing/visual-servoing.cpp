...
int main(int argc, char* argv[]) {
  ...
  Point goto_xy_request;

  double max_vel_x = 250.0;
  double max_vel_y = 250.0;
  double gain_x = 250.0 / 800.0;
  ...
  auto thread = is::advertise(uri, name, {
    ...
    {
      "go_to_xy", [&](is::Request request) -> is::Reply {
        mtx.lock();
        goto_xy_request = is::msgpack<Point>(request);
        goto_type = GO_TO_XY;
        set_desired_pose.store(true);
        mtx.unlock();
        return is::msgpack(status::ok);
  ...
  auto loop_interval = milliseconds(static_cast<int>(1000.0 / *sample_rate.rate));
  ...
  while (1) {
    ...
    case ControllerState::CONSUME_IMAGE: {
      ...
      is::log::info("got frames");

      deadline = system_clock::now() + loop_interval;
    ...
    case ControllerState::SET_DESIRED_POSE: {
      ...
      case GO_TO: {
        ...
        if (reply != nullptr) {
          ...
          if (points3d.points.size() == 1) {
            ...
            is::logger()->info("Setting new desired pose (go_to): {},{}", desired_pose.position.x,
      ...
      case GO_TO_XY: {
        mtx.lock();
        desired_pose = {{goto_xy_request.x, goto_xy_request.y}, 0.0};
        mtx.unlock();
        is::log::info("Setting new desired pose (go_to_xy): {},{}", desired_pose.position.x, desired_pose.position.y);
        break;
      }
      }
      fence();
    ...
    case ControllerState::CONTROL_ACTION: {
      ...
      if (reply == nullptr) {
        ...
        controller_state = ControllerState::NEW_CONFIGURE;
        n_deadlines = 0;
        is::log::info("Processing took: {}", duration_cast<milliseconds>(high_resolution_clock::now() - start).count());
      }
      break;
    }
